...
async function loadFileByPath(fullFileName) {
  try {
    ...
    const expectedHash = `#${encodeURIComponent(
      bucketName
    ...
    if (fileContentCache.has(encodedFileName)) {
      ...
      currentFileContent = JSON.stringify(jsonData);
    ...
    if (file) {
      ...
      document.getElementById("fileDate").textContent = new Date(
        file.lastModified
    ...
    document.getElementById("logContent").innerHTML =
      '<div class="placeholder">Error loading file</div>';
  }
}
...
async function loadBuckets() {
  try {
    ...
    document.getElementById("itemList").innerHTML =
      '<div class="loading">Error loading date buckets</div>';
  }
}
...
function renderBuckets() {
  ...
  itemList.innerHTML = currentBuckets
    .map(
      (bucket) => `
  ...
  try {
    ...
    if (filesCache.has(bucketName)) {
      ...
      const response = await fetch(
        `/api/dates/${encodeURIComponent(bucketName)}/files`
      );
    ...
    document.getElementById(
      "sidebarTitle"
    ).textContent = `Files in ${bucketName}`;
    ...
    if (!preserveSelection) {
      ...
      document.getElementById("logContent").innerHTML =
        '<div class="placeholder">Select a JSON file from the left to view its contents</div>';
    } else if (
      previousSelectedFile &&
      currentFiles.some((f) => f.name === previousSelectedFile)
    ) {
...
function renderFiles() {
  ...
  if (currentFiles.length === 0) {
    itemList.innerHTML =
      '<div class="loading">No JSON files found in this date bucket</div>';
    return;
  }

  itemList.innerHTML = currentFiles
    .map(
      (file) => `
        <div class="file-item" onclick="navigateToFile('${encodeURIComponent(
          file.name
        )}')">
            <div class="file-name">${file.displayName}</div>
            <div class="file-meta">
                <span>${formatFileSize(file.size)}</span>
                <span>${new Date(file.lastModified).toLocaleTimeString(
                  "en-GB",
                  { hour12: false }
                )}</span>
            </div>
