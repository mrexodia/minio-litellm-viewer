...
const minioClient = new Client({
  ...
  port: parseInt(bucketUrl.port) || (bucketUrl.protocol === 'https:' ? 443 : 80),
...
app.get('/api/buckets', async (req, res) => {
  try {
    ...
    stream.on('end', () => {
      console.log(`Found ${dateFolders.size} date folders:`, Array.from(dateFolders));
      const sortedFolders = Array.from(dateFolders).sort((a, b) => b.localeCompare(a));
      res.json(sortedFolders);
    });

    stream.on('error', (err) => {
      console.error('Error listing date folders:', err);
      res.status(500).json({ error: 'Failed to list date folders', details: err.message });
    });
  } catch (error) {
    console.error('Error in /api/buckets:', error);
    res.status(500).json({ error: 'Internal server error', details: error.message });
  }
});

app.get('/api/files/:dateBucket', async (req, res) => {
  try {
    const dateBucket = req.params.dateBucket;
    const objects = [];
    const stream = minioClient.listObjects(bucketName, dateBucket + '/', false);
    ...
    stream.on('end', () => {
      res.json(objects.sort((a, b) => new Date(b.lastModified) - new Date(a.lastModified)));
    });
